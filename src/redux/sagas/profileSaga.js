    ...
    case UPDATE_PROFILE: {
      try {
        ...
        if (avatarFile || bannerFile) {
          ...
          if (bannerFile) {
            updates.banner = yield call(firebase.storeImage, state.auth.id, 'banner', bannerFile);
          }

          if (avatarFile) {
            updates.avatar = yield call(firebase.storeImage, state.auth.id, 'avatar', avatarFile);
          }
